        ...
        <div className="container max-w-5xl">
          <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2">
              {!submitted ? (
                <>
                      ...
                      <div key={label} className="flex items-center gap-1 flex-1">
                        <div className={`flex items-center gap-2 flex-1 ${i < stepLabels.length - 1 ? "" : ""}`}>
                          <span className={`w-7 h-7 rounded-full text-xs flex items-center justify-center font-medium shrink-0 transition-colors ${
                            step > i + 1 ? "bg-[#5B8A72] text-white" : step === i + 1 ? "bg-[#1E3A5F] text-white" : "bg-[#e8e3db] text-[#6B7280]"
                          }`}>
                            {step > i + 1 ? <CheckCircle2 className="w-3.5 h-3.5" /> : i + 1}
                          </span>
                          <span className={`text-xs hidden sm:block ${step === i + 1 ? "text-[#1A1A2E] font-medium" : "text-[#6B7280]"}`}>{label}</span>
                        </div>
                  ...
                  {/* Card */}
                  <div className="border border-[#e8e3db] rounded-2xl overflow-hidden">
                    <div className="px-6 py-4 bg-[#F9F7F4] border-b border-[#e8e3db]">
                    ...
                    <div className="p-6">
                      <AnimatePresence mode="wait">
                        ...
                        {step === 1 && (
                          <motion.div key="s1" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }} transition={{ duration: 0.2 }} className="space-y-6">
                            <div>
                              <label className="text-xs text-[#6B7280] mb-2 block font-medium">I am donating as</label>
                              <div className="flex gap-3">
                                {([
                                  ...
                                  <button key={d.id} type="button" onClick={() => set("donorType", d.id)}
                                    className={`flex-1 p-4 rounded-xl border text-left transition-all ${form.donorType === d.id ? "border-[#1E3A5F] bg-[#1E3A5F]/5" : "border-[#e8e3db] hover:border-[#d4cfc7]"}`}>
                                    <d.icon className={`w-5 h-5 mb-2 ${form.donorType === d.id ? "text-[#1E3A5F]" : "text-[#d4cfc7]"}`} />
                                    <p className={`text-sm font-medium ${form.donorType === d.id ? "text-[#1E3A5F]" : "text-[#1A1A2E]"}`}>{d.label}</p>
                                    <p className="text-[10px] text-[#6B7280] mt-0.5">{d.sub}</p>
                                  </button>
                                ))}
                              </div>
                            </div>
                            <div>
                              <label className="text-xs text-[#6B7280] mb-2 block font-medium">Frequency</label>
                              <div className="flex gap-2">
                                {([{ id: "onetime" as const, label: "One-time" }, { id: "monthly" as const, label: "Monthly" }] as const).map((f) => (
                                  <button key={f.id} type="button" onClick={() => set("frequency", f.id)}
                                    className={`flex-1 h-11 rounded-lg text-sm border transition-all ${form.frequency === f.id ? "border-[#1E3A5F] bg-[#1E3A5F]/5 text-[#1E3A5F] font-medium" : "border-[#e8e3db] text-[#6B7280]"}`}>
                                    {f.label}
                                  </button>
                                ))}
                              </div>
                            ...
                            <div>
                              <label className="text-xs text-[#6B7280] mb-2 block font-medium">Select an amount</label>
                              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mb-3">
                                {presetAmounts.map((a) => (
                                  <button key={a.value} type="button" onClick={() => { set("amount", a.value); set("customAmount", ""); }}
                                    className={`p-3 rounded-lg border text-left transition-all ${form.amount === a.value && !form.customAmount ? "border-[#1E3A5F] bg-[#1E3A5F]/5" : "border-[#e8e3db] hover:border-[#d4cfc7]"}`}>
                                    <p className={`text-sm font-medium ${form.amount === a.value && !form.customAmount ? "text-[#1E3A5F]" : "text-[#1A1A2E]"}`}>{a.label}</p>
                                    <p className="text-[10px] text-[#6B7280] mt-0.5">{a.impact}</p>
                                  </button>
                                ))}
                              </div>
                              <div className="relative">
                                ...
                                <Input type="number" min="100" value={form.customAmount}
                                  ...
                                  placeholder="Or enter a custom amount" className="bg-white border-[#e8e3db] text-sm h-11 rounded-lg pl-7" />
                              </div>
                            </div>
                          </motion.div>
                        )}
                        ...
                        {step === 2 && (
                          <motion.div key="s2" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }} transition={{ duration: 0.2 }} className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                              <div>
                                <label className="text-xs text-[#6B7280] mb-1.5 block font-medium">Email *</label>
                              ...
                              <div>
                                <label className="text-xs text-[#6B7280] mb-1.5 block font-medium">Phone *</label>
                            ...
                            <div>
                              <label className="text-xs text-[#6B7280] mb-1.5 block font-medium">PAN number <span className="text-[#d4cfc7]">(for 80G receipt)</span></label>
                              <Input value={form.pan} onChange={(e) => set("pan", e.target.value.toUpperCase())} placeholder="ABCDE1234F" maxLength={10} className="bg-white border-[#e8e3db] text-sm h-11 rounded-lg uppercase" />
                            </div>
                            {form.donorType === "corporate" && (
                              <div className="pt-3 border-t border-[#e8e3db] space-y-4">
                                <p className="text-xs text-[#C69C3F] font-medium">CSR Details</p>
                                <div>
                                  <label className="text-xs text-[#6B7280] mb-1.5 block font-medium">Company name *</label>
                                  <Input value={form.companyName} onChange={(e) => set("companyName", e.target.value)} placeholder="Company registered name" className="bg-white border-[#e8e3db] text-sm h-11 rounded-lg" />
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                  <div>
                                    <label className="text-xs text-[#6B7280] mb-1.5 block font-medium">CIN number</label>
                                    <Input value={form.cinNumber} onChange={(e) => set("cinNumber", e.target.value)} placeholder="U12345DL2020PTC123456" className="bg-white border-[#e8e3db] text-sm h-11 rounded-lg" />
                                  </div>
                                  <div>
                                    <label className="text-xs text-[#6B7280] mb-1.5 block font-medium">CSR contact person</label>
                                    <Input value={form.csrContact} onChange={(e) => set("csrContact", e.target.value)} placeholder="Name of CSR head" className="bg-white border-[#e8e3db] text-sm h-11 rounded-lg" />
                                  </div>
                                </div>
                              </div>
                            )}
                          ...
                          <motion.div key="s3" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }} transition={{ duration: 0.2 }} className="space-y-5">
                            <div className="space-y-3">
                              <button type="button" onClick={() => set("paymentMethod", "upi")}
                                className={`w-full p-4 rounded-xl border text-left transition-all ${form.paymentMethod === "upi" ? "border-[#1E3A5F] bg-[#1E3A5F]/5" : "border-[#e8e3db] hover:border-[#d4cfc7]"}`}>
                                <p className={`text-sm font-medium ${form.paymentMethod === "upi" ? "text-[#1E3A5F]" : "text-[#1A1A2E]"}`}>UPI Transfer</p>
                                <p className="text-[11px] text-[#6B7280] mt-0.5">Pay via Google Pay, PhonePe, Paytm, or any UPI app</p>
                              </button>
                              <button type="button" onClick={() => set("paymentMethod", "bank")}
                                className={`w-full p-4 rounded-xl border text-left transition-all ${form.paymentMethod === "bank" ? "border-[#1E3A5F] bg-[#1E3A5F]/5" : "border-[#e8e3db] hover:border-[#d4cfc7]"}`}>
                                <p className={`text-sm font-medium ${form.paymentMethod === "bank" ? "text-[#1E3A5F]" : "text-[#1A1A2E]"}`}>Bank Transfer (NEFT / RTGS / IMPS)</p>
                                <p className="text-[11px] text-[#6B7280] mt-0.5">Direct transfer to our registered bank account</p>
                              </button>
                            </div>
                            {form.paymentMethod === "upi" && (
                              <div className="border border-[#e8e3db] rounded-xl p-4 space-y-3">
                                ...
                                <p className="text-[11px] text-[#6B7280]">Copy the UPI ID and pay ₹{selectedAmount.toLocaleString("en-IN")} from your preferred app.</p>
                              </div>
                            )}
                            {form.paymentMethod === "bank" && (
                              <div className="border border-[#e8e3db] rounded-xl p-4 space-y-2">
                                ...
                                {[
                                  ...
                                  <div key={row.label} className="flex items-center justify-between py-2 border-b border-[#e8e3db] last:border-0">
                                    <div>
                                      <p className="text-[11px] text-[#6B7280]">{row.label}</p>
                                      <p className="text-sm text-[#1A1A2E] font-medium">{row.value}</p>
                                    </div>
                          ...
                          <motion.div key="s4" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }} transition={{ duration: 0.2 }} className="space-y-5">
                            ...
                            <div className="bg-[#C69C3F]/10 border border-[#C69C3F]/20 p-4 rounded-xl">
                              <p className="text-xs text-[#6B7280] leading-relaxed">
                                <span className="text-[#1A1A2E] font-medium">80G Tax Benefit: </span>
                                Your donation is eligible for tax deduction under Section 80G. An 80G receipt will be emailed to <span className="text-[#1A1A2E] font-medium">{form.email}</span> within 24 hours.
                              </p>
                            </div>
                            {form.donorType === "corporate" && (
                      ...
                      {/* Navigation */}
                      <div className="mt-6 pt-5 border-t border-[#e8e3db] flex justify-between items-center">
                        {step > 1 ? (
                          <button onClick={() => setStep((s) => s - 1)} className="flex items-center gap-1 text-sm text-[#6B7280] hover:text-[#1A1A2E]">
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                  className="border border-[#e8e3db] rounded-2xl p-10 text-center">
                  ...
                  <h2 className="text-xl font-medium text-[#1A1A2E] mb-2">Thank you, {form.fullName.split(" ")[0]}</h2>
                  <p className="text-sm text-[#6B7280] mb-2 max-w-sm mx-auto">
                    Your {form.frequency === "monthly" ? "monthly " : ""}donation of <span className="text-[#1A1A2E] font-medium">₹{selectedAmount.toLocaleString("en-IN")}</span> will directly fund free legal representation for citizens who cannot afford justice.
                  </p>
                  <p className="text-xs text-[#6B7280] mb-6">
                    An 80G receipt will be emailed to <span className="text-[#1A1A2E] font-medium">{form.email}</span> within 24 hours.
                  </p>
                  <div className="inline-block bg-[#F9F7F4] border border-[#e8e3db] rounded-xl p-4 mb-6 text-left">
                    <p className="text-[11px] text-[#6B7280] mb-1">Donation Reference</p>
                    <p className="text-lg font-medium text-[#1E3A5F]">DON-{new Date().getFullYear()}-{String(Math.floor(Math.random() * 9000) + 1000)}</p>
                    <p className="text-[11px] text-[#6B7280] mt-1">Mention this reference when making the transfer.</p>
                  </div>
            ...
            <div className="lg:col-span-1 space-y-5">
              {/* Fund allocation */}
              <div className="border border-[#e8e3db] rounded-xl p-5">
                <h3 className="text-sm font-medium text-[#1A1A2E] mb-4">Where your money goes</h3>
                <div className="space-y-3">
                  {fundAllocation.map((item) => (
                    <div key={item.label}>
                      <div className="flex justify-between text-xs mb-1">
                        <span className="text-[#6B7280]">{item.label}</span>
                        <span className="text-[#1A1A2E] font-medium">{item.pct}%</span>
                      </div>
                      <div className="h-1.5 bg-[#F9F7F4] rounded-full overflow-hidden">
              ...
              {/* Trust badges */}
              <div className="border border-[#e8e3db] rounded-xl p-5 space-y-3">
                <h3 className="text-sm font-medium text-[#1A1A2E] mb-1">Tax & compliance</h3>
                <div className="flex items-start gap-2.5">
                  <Shield className="w-4 h-4 text-[#C69C3F] mt-0.5 shrink-0" />
                  <div>
                    <p className="text-xs font-medium text-[#1A1A2E]">80G Tax Exemption</p>
                    <p className="text-[11px] text-[#6B7280]">Donations are eligible for tax deduction under Section 80G of the IT Act</p>
                  </div>
                </div>
                <div className="flex items-start gap-2.5">
                  <Receipt className="w-4 h-4 text-[#C69C3F] mt-0.5 shrink-0" />
                  <div>
                    <p className="text-xs font-medium text-[#1A1A2E]">12A Registered</p>
                    <p className="text-[11px] text-[#6B7280]">Registered non-profit under Section 12A</p>
                  </div>
                </div>
                <div className="flex items-start gap-2.5">
                  <Building2 className="w-4 h-4 text-[#C69C3F] mt-0.5 shrink-0" />
                  <div>
                    <p className="text-xs font-medium text-[#1A1A2E]">CSR Eligible</p>
                    <p className="text-[11px] text-[#6B7280]">Qualifies under Schedule VII, Companies Act 2013</p>
                  </div>
                </div>
              </div>
              ...
              {/* Contact */}
              <div className="border border-[#e8e3db] rounded-xl p-5">
                <h3 className="text-sm font-medium text-[#1A1A2E] mb-2">Need help?</h3>
                <p className="text-xs text-[#6B7280] mb-3">For CSR partnerships or large donations, reach out directly.</p>
