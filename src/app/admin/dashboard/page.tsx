...
export default function AdminDashboard() {
  ...
  const statusStyles: Record<string, string> = {
    ...
    "Verified": "bg-primary/10 text-[#1E3A5F] border-[#1E3A5F]/20",
  ...
  return (
    <div className="min-h-screen bg-muted">
      ...
      <main className="max-w-6xl mx-auto px-6 py-8">
        <div className="mb-8">
          <h1 className="text-xl font-medium text-foreground mb-1">Welcome, Admin</h1>
          <p className="text-sm text-muted-foreground">Here&apos;s an overview of current operations.</p>
        </div>
        ...
        <div className="bg-background border border-border rounded-xl overflow-hidden">
          <div className="px-5 py-3.5 border-b border-border flex justify-between items-center">
            ...
            <span className="text-xs text-muted-foreground">{applications.length} shown</span>
          </div>
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-border hover:bg-transparent">
                  ...
                  <TableHead className="text-[11px] text-muted-foreground uppercase tracking-wider">Applicant</TableHead>
                  <TableHead className="text-[11px] text-muted-foreground uppercase tracking-wider">Category</TableHead>
                  <TableHead className="text-[11px] text-muted-foreground uppercase tracking-wider">Income</TableHead>
                  <TableHead className="text-[11px] text-muted-foreground uppercase tracking-wider">Date</TableHead>
              ...
              <TableBody>
                {applications.map((app) => (
                  <TableRow key={app.id} className="border-border hover:bg-muted">
                    <TableCell className="text-sm font-medium text-foreground font-mono text-xs">{app.id}</TableCell>
                    <TableCell className="text-sm text-foreground">{app.name}</TableCell>
                    <TableCell className="text-sm text-muted-foreground">{app.category}</TableCell>
                    <TableCell className="text-sm text-muted-foreground">{app.income}</TableCell>
                    ...
                    <TableCell className="text-right">
                      <Button variant="ghost" size="sm" className="text-[#1E3A5F] hover:bg-primary/10 text-xs h-7">Review</Button>
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
