      ...
      {isOpen && (
        <>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.97 }}
            ...
            className="fixed left-1/2 top-1/2 z-[101] -translate-x-1/2 -translate-y-1/2 w-full max-w-[340px] bg-background border border-border rounded-2xl p-6">
            <button onClick={onClose} className="absolute top-3 right-3 p-1.5 text-muted-foreground hover:text-foreground hover:bg-muted rounded-full transition-colors" aria-label="Close">
              <X className="h-4 w-4" />
            </button>
            ...
            <div className="text-center mb-5">
              ...
              <h2 className="text-base font-medium text-foreground">Sign in to IPBLI</h2>
              <p className="text-xs text-muted-foreground mt-1">Choose a role to explore the platform</p>
            ...
            {!loading ? (
              <div className="space-y-2">
                <button onClick={() => handleLogin("ADMIN")}
                  ...
                  <div><p className="text-sm font-medium text-foreground">Admin</p><p className="text-[11px] text-muted-foreground">Operations dashboard</p></div>
                </button>
                <button onClick={() => handleLogin("ADVOCATE")}
                  ...
                  <div><p className="text-sm font-medium text-foreground">Advocate</p><p className="text-[11px] text-muted-foreground">Manage pro bono cases</p></div>
                </button>
                <button onClick={() => handleLogin("BENEFICIARY")}
                  ...
                  <div><p className="text-sm font-medium text-foreground">Citizen</p><p className="text-[11px] text-muted-foreground">Apply or track your case</p></div>
                </button>
              </div>
            ) : (
